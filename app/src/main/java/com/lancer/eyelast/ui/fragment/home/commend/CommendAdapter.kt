        ...
        when(holder){
            is TextCardViewHeader5ViewHolder -> {
               ...
               // holder.tvFollow.setOnClickListener { LoginActivity.start(fragment.activity) }
                setOnClickListener(holder.tvTitle5, holder.ivInto5) { ActionUrl.process(fragment, item.data.actionUrl, item.data.text) }
            }
            is TextCardViewHeader7ViewHolder -> {
                ...
                setOnClickListener(holder.tvRightText7, holder.ivInto7) {
                    ActionUrl.process(fragment, item.data.actionUrl, "${item.data.text},${item.data.rightText}")
                }
            }
            is TextCardViewHeader8ViewHolder -> {
                holder.tvTitle8.text = item.data.text
                holder.tvRightText8.text = item.data.rightText
                setOnClickListener(holder.tvRightText8, holder.ivInto8) { ActionUrl.process(fragment, item.data.actionUrl, item.data.text) }
            }
            is TextCardViewFooter2ViewHolder -> {
                holder.tvFooterRightText2.text = item.data.text
                setOnClickListener(holder.tvFooterRightText2, holder.ivTooterInto2) { ActionUrl.process(fragment, item.data.actionUrl, item.data.text) }
            }
            is TextCardViewFooter3ViewHolder -> {
                ...
                setOnClickListener(holder.tvRefresh, holder.tvFooterRightText3, holder.ivTooterInto3) {
                    if (this == holder.tvRefresh) {
                        "${holder.tvRefresh.text},该功能即将开放，敬请期待".showToast()
...
//                    }
            ...
            is Banner3ViewHolder -> {
                ...
                holder.itemView.setOnClickListener { ActionUrl.process(fragment, item.data.actionUrl, item.data.header.title) }
            }
            is InformationCardFollowCardViewHolder -> {
                ...
                holder.recyclerView.adapter = fragment.activity?.let {
                    InformationCardFollowCardAdapter(
                        it, item.data.actionUrl, item.data.titleList)
                }
                holder.itemView.setOnClickListener { ActionUrl.process(fragment, item.data.actionUrl) }
            }
...
//                }
            ...
            is AutoPlayVideoAdViewHolder -> {
                item.data.detail?.run {
                    ...
                    fragment.activity?.let {
                        startAutoPlay(it, holder.videoPlayer, position, url, imageUrl, TAG, object : GSYSampleCallBack() {
    ...
    class InformationCardFollowCardAdapter(val activity: Activity, val actionUrl: String?, val dataList: List<String>) :
        ...
        override fun onBindViewHolder(holder: InformationCardFollowCardAdapter.ViewHolder, position: Int) {
            ...
            holder.itemView.setOnClickListener { ActionUrl.process(activity, actionUrl) }
        }
    }
